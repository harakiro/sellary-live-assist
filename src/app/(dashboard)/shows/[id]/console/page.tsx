...
export default function ConsolePage() {
  ...
  const fetchComments = useCallback(async () => {
    ...
    if ('data' in res) setCommentEntries(res.data);
  }, [showId]);
    ...
    switch (event.type) {
      ...
      case 'comment.received':
        setCommentEntries((prev) => [
          {
            id: event.data.commentId || `c-${Date.now()}`,
            userHandle: event.data.userHandle,
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col -m-8">
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-80 border-r overflow-y-auto bg-white">
          <ItemsPanel items={show.items} showId={showId} onRefresh={fetchShow} />
        </div>
        <div className="flex-1 overflow-y-auto bg-white">
          <ClaimFeed entries={claimEntries} />
        </div>
